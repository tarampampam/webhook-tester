...
import { useStorage, UsedStorageKeys, type StorageArea, useAppConfig, useSessions } from '~/shared'
...
export const NewSessionModal = ({ opened, onClose }: { opened: boolean; onClose: () => void }): React.JSX.Element => {
  const navigate = useNavigate()
  const { config } = useAppConfig()
  const maxBodySize = config?.limits.maxRequestBodySize ?? null
  const { newSession, delSession } = useSessions()
  ...
  const statusVal = status ?? controls.code.def
  const headersVal = headers ?? controls.head.def
  const delayVal = delay ?? controls.delay.def
  const bodyVal = body ?? controls.body.def
  const destroyVal = destroy ?? controls.destroy.def
  ...
  const wrongResponseBody = useMemo(() => {
    ...
    if (!!maxBodySize && bodyIsValid) {
      bodyIsValid = body !== null && body.length <= maxBodySize
    }
  ...
  const handleCreate = () => {
    ...
    const respHeaders: { [k: string]: string } = Object.fromEntries(
      headersTextToHeaders(headersVal).map((h) => [h.name, h.value])
    )
    ...
    newSession({
      ...
      delay: delayVal > 0 ? delayVal : undefined,
      responseBody: bodyVal.trim().length > 0 ? new TextEncoder().encode(bodyVal) : undefined,
    })
      .then((newSID) => {
        notify.update({
          id,
